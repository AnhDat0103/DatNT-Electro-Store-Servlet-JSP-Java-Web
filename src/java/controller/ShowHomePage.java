 ...
 * @author DAT
 */
@WebServlet(name = "ShowHomePage", urlPatterns = {""})
public class ShowHomePage extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        int topRows = 8;
        List<Product> products = pd.getTopNewProduct(topRows);
        if (!products.isEmpty()) {
            request.setAttribute("products", products);
        } else {
            request.setAttribute("errorNewProducts", "Không tìm thấy sản phẩm nào.");
        }
